...
export async function createDoctor(_prevState: any, formData: FormData) {
    try {
        const specialitiesValue = formData.get('specialities') as string;
        ...
        const newPayload = {
            ...
            specialities: specialitiesValue ? [specialitiesValue] : [],
        };
        ...
        const response = await serverFetch.post('/doctor/create-doctor', {
            body: newFormData,
        });
...
export async function getDoctors(queryString?: string) {
    try {
        const response = await serverFetch.get(
            `/doctor/get-all-doctors${queryString ? `?${queryString}` : ''}`
        );
        const result = await response.json();
        return result;
    } catch (error: any) {
